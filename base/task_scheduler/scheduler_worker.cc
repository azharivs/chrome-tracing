...
/*sva begin*/
        ...
        if (!soLoaded){
            tppHandle = dlopen("/home/iustack/libchrome-sch-worker-tp.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle) {
        ...
        *(void **) (&tp_ptr) = dlsym(tppHandle, "_Z16trpoint_functioniPc");
        ...
        (*tp_ptr)((int) tid, (char *) "SchedulerWorker::SchedulerWorker");
...
/*sva begin*/
        ...
        void (*tp_ptr)(int,bool,bool);
        char *error;

        if (!soLoaded){
            tppHandle = dlopen("/home/iustack/libchrome-sch-worker-tp.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle) {
                 fprintf(stderr, "01: Upon dlopen: %s\n", dlerror());
            }
            else {
                //printf("--- 01: TRACEPOINT REGISTERED\n");
                soLoaded = 1;
            }
        }
        ...
        *(void **) (&tp_ptr) = dlsym(tppHandle, "_Z20trpoint_worker_startibb");

        if ((error = dlerror()) != NULL)  {
             fprintf(stderr, "01: Upon dlsym: %s\n", error);
        }

        //pid_t pid = getpid();
        pid_t tid = syscall(SYS_gettid);
        (*tp_ptr)((int) tid, should_exit_.IsSet(), false );
        //std::cout << "....." << (int) tid << ": dlsym\n";
...
/*sva end*/
  ...
  (*tp_ptr)((int) tid, should_exit_.IsSet(), thread_handle_.is_null());
...
/*sva begin*/
        ...
        if (!soLoaded){
            tppHandle = dlopen("/home/iustack/libchrome-sch-worker-tp.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle) {
                 fprintf(stderr, "02: Upon dlopen: %s\n", dlerror());
            }
            else {
                //printf("--- 02: TRACEPOINT REGISTERED\n");
                soLoaded = 1;
            }
        }
        ...
        *(void **) (&tp_ptr) = dlsym(tppHandle, "_Z21trpoint_worker_wakeupi");

        if ((error = dlerror()) != NULL)  {
             fprintf(stderr, "02: Upon dlsym: %s\n", error);
        }
...
/*sva begin*/
        ...
        if (!soLoaded){ 
            tppHandle = dlopen("/home/iustack/libchrome-sch-worker-tp.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle) {
                 fprintf(stderr, "03: Upon dlopen: %s\n", dlerror());
            }
            else {
                //printf("--- 03: TRACEPOINT REGISTERED\n");
                soLoaded = 1;
            }
        }
        ...
        *(void **) (&tp_ptr) = dlsym(tppHandle, "_Z16trpoint_functioniPc");
        ...
        //pid_t pid = getpid();
        pid_t tid = syscall(SYS_gettid);
        (*tp_ptr)((int) tid, (char *) "SchedulerWorker::RunWorker");
        //std::cout << "....." << (int) tid << ": dlsym\n";
