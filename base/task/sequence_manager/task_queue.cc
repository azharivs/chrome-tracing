...
void TaskQueue::TaskTiming::RecordTaskStart(LazyNow* now) {
  TRACE_EVENT0("dorsal", "TaskQueue::TaskTiming::RecordTaskStart");
  if (has_wall_time())
    start_time_ = now->Now();
  if (has_thread_time())
    start_thread_time_ = base::ThreadTicks::Now();
  TRACE_EVENT0("dorsal", "TaskQueue::TaskTiming::RecordTaskStart_END");
}

void TaskQueue::TaskTiming::RecordTaskEnd(LazyNow* now) {
  TRACE_EVENT0("dorsal", "TaskQueue::TaskTiming::RecordTaskEnd");
  if (has_wall_time())
    end_time_ = now->Now();
  if (has_thread_time())
    end_thread_time_ = base::ThreadTicks::Now();
  TRACE_EVENT0("dorsal", "TaskQueue::TaskTiming::RecordTaskEnd_END");
}
...
void TaskQueue::ShutdownTaskQueue() {
  TRACE_EVENT0("dorsal", "TaskQueue::ShutdownTaskQueue");
  DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  AutoLock lock(impl_lock_);
  if (!impl_){
  TRACE_EVENT0("dorsal", "TaskQueue::ShutdownTaskQueue_END");
    return;}
...
bool TaskQueue::RunsTasksInCurrentSequence() const {
  TRACE_EVENT0("dorsal", "TaskQueue::RunsTasksInCurrentSequence");
  bool temp;
  temp = IsOnMainThread();
  TRACE_EVENT0("dorsal", "TaskQueue::RunsTasksInCurrentSequence_END");
  return temp;
...
bool TaskQueue::PostDelayedTask(const Location& from_here,
                                OnceClosure task,
                                TimeDelta delay) {
  TRACE_EVENT0("dorsal", "TaskQueue::PostDelayedTask");
  bool temp;
  temp = PostTaskWithMetadata(
      PostedTask(std::move(task), from_here, delay, Nestable::kNestable));
  TRACE_EVENT0("dorsal", "TaskQueue::PostDelayedTask_END");
  return temp;
...
bool TaskQueue::PostNonNestableDelayedTask(const Location& from_here,
                                           OnceClosure task,
                                           TimeDelta delay) {
  TRACE_EVENT0("dorsal", "TaskQueue::PostNonNestableDelayedTask");
  bool temp;
  temp = PostTaskWithMetadata(
      PostedTask(std::move(task), from_here, delay, Nestable::kNonNestable));
  TRACE_EVENT0("dorsal", "TaskQueue::PostNonNestableDelayedTask_END");
  return temp;
...
bool TaskQueue::PostTaskWithMetadata(PostedTask task) {
  TRACE_EVENT0("dorsal", "TaskQueue::PostTaskWithMetadata");
  Optional<MoveableAutoLock> lock = AcquireImplReadLockIfNeeded();
  if (!impl_){
  TRACE_EVENT0("dorsal", "TaskQueue::PostTaskWithMetadata_END");
    return false;}
...
bool TaskQueue::IsQueueEnabled() const {
  TRACE_EVENT0("dorsal", "TaskQueue::IsQueueEnabled");
  DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  bool temp;
  temp = impl_->IsQueueEnabled();
  TRACE_EVENT0("dorsal", "TaskQueue::IsQueueEnabled_END");
...
size_t TaskQueue::GetNumberOfPendingTasks() const {
  TRACE_EVENT0("dorsal", "TaskQueue::GetNumberOfPendingTasks");
  DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  bool temp;
  temp = impl_->GetNumberOfPendingTasks();
  TRACE_EVENT0("dorsal", "TaskQueue::GetNumberOfPendingTasks_END");
...
bool TaskQueue::IsOnMainThread() const {
  TRACE_EVENT0("dorsal", "TaskQueue::IsOnMainThread");
  bool temp;
  temp = associated_thread_->thread_id == PlatformThread::CurrentId();
  TRACE_EVENT0("dorsal", "TaskQueue::IsOnMainThread_END");
  return temp;
