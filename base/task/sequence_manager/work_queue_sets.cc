...
void WorkQueueSets::AddQueue(WorkQueue* work_queue, size_t set_index) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::AddQueue");
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::AddQueue_END");
    return;}
...
void WorkQueueSets::RemoveQueue(WorkQueue* work_queue) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::RemoveQueue");
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::RemoveQueue_END");
    return;}
...
void WorkQueueSets::ChangeSetIndex(WorkQueue* work_queue, size_t set_index) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::ChangeSetIndex");
  DCHECK_EQ(this, work_queue->work_queue_sets());
  DCHECK_LT(set_index, work_queue_heaps_.size());
  EnqueueOrder enqueue_order;
  bool has_enqueue_order = work_queue->GetFrontTaskEnqueueOrder(&enqueue_order);
  ...
  work_queue->AssignSetIndex(set_index);
  if (!has_enqueue_order){
  TRACE_EVENT0("dorsal", "WorkQueueSets::ChangeSetIndex_END");
    return;}
...
void WorkQueueSets::OnFrontTaskChanged(WorkQueue* work_queue) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnFrontTaskChanged");
  EnqueueOrder enqueue_order;
  bool has_enqueue_order = work_queue->GetFrontTaskEnqueueOrder(&enqueue_order);
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnFrontTaskChanged_END");
}

void WorkQueueSets::OnTaskPushedToEmptyQueue(WorkQueue* work_queue) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnTaskPushedToEmptyQueue");
  ...
  work_queue_heaps_[set_index].insert({enqueue_order, work_queue});
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnTaskPushedToEmptyQueue_END");
}

void WorkQueueSets::OnPopQueue(WorkQueue* work_queue) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnPopQueue");
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnPopQueue_END");
}

void WorkQueueSets::OnQueueBlocked(WorkQueue* work_queue) {
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnQueueBlocked");
  DCHECK_EQ(this, work_queue->work_queue_sets());
  HeapHandle heap_handle = work_queue->heap_handle();
  if (!heap_handle.IsValid()){
  TRACE_EVENT0("dorsal", "WorkQueueSets::OnQueueBlocked_END");
    return;}
  size_t set_index = work_queue->work_queue_set_index();
  DCHECK_LT(set_index, work_queue_heaps_.size());
  work_queue_heaps_[set_index].erase(heap_handle);
...
bool WorkQueueSets::GetOldestQueueInSet(size_t set_index,
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::GetOldestQueueInSet");
  DCHECK_LT(set_index, work_queue_heaps_.size());
  if (work_queue_heaps_[set_index].empty()){
  TRACE_EVENT0("dorsal", "WorkQueueSets::GetOldestQueueInSet_END");
    return false;}
...
bool WorkQueueSets::GetOldestQueueAndEnqueueOrderInSet(
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::GetOldestQueueAndEnqueueOrderInSet");
  DCHECK_LT(set_index, work_queue_heaps_.size());
  if (work_queue_heaps_[set_index].empty()){
  TRACE_EVENT0("dorsal", "WorkQueueSets::GetOldestQueueAndEnqueueOrderInSet_END");
    return false;}
...
bool WorkQueueSets::IsSetEmpty(size_t set_index) const {
  TRACE_EVENT0("dorsal", "WorkQueueSets::IsSetEmpty");
  DCHECK_LT(set_index, work_queue_heaps_.size())
      << " set_index = " << set_index;
  TRACE_EVENT0("dorsal", "WorkQueueSets::IsSetEmpty_END");
...
bool WorkQueueSets::ContainsWorkQueueForTest(
  ...
  TRACE_EVENT0("dorsal", "WorkQueueSets::ContainsWorkQueueForTest");
  EnqueueOrder enqueue_order;
  bool has_enqueue_order = work_queue->GetFrontTaskEnqueueOrder(&enqueue_order);
  ...
  for (const IntrusiveHeap<OldestTaskEnqueueOrder>& heap : work_queue_heaps_) {
    for (const OldestTaskEnqueueOrder& heap_value_pair : heap) {
      if (heap_value_pair.value == work_queue) {
        ...
        DCHECK_EQ(this, work_queue->work_queue_sets());
  TRACE_EVENT0("dorsal", "WorkQueueSets::ContainsWorkQueueForTest_END");
        return true;
      }
    }
  }
