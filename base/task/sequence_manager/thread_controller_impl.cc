...
void ThreadControllerImpl::SetSequencedTaskSource(
  ...
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetSequencedTaskSource");
  DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
  DCHECK(sequence);
  DCHECK(!sequence_);
  sequence_ = sequence;
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetSequencedTaskSource_END");
}

void ThreadControllerImpl::SetTimerSlack(TimerSlack timer_slack) {
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetTimerSlack");
  if (!message_loop_){
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetTimerSlack_END");
    return;}
...
void ThreadControllerImpl::ScheduleWork() {
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::ScheduleWork");
  DCHECK(sequence_);
  ...
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::ScheduleWork_END");
    return;
  }
...
void ThreadControllerImpl::SetNextDelayedDoWork(LazyNow* lazy_now,
                                                TimeTicks run_time) {
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetNextDelayedDoWork");
  DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
  DCHECK(sequence_);

  if (main_sequence_only().next_delayed_do_work == run_time){
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetNextDelayedDoWork_END");
    return;}
...
bool ThreadControllerImpl::RunsTasksInCurrentSequence() {
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::RunsTasksInCurrentSequence");
  bool temp;
  temp = task_runner_->RunsTasksInCurrentSequence();
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::RunsTasksInCurrentSequence_END");
  return temp;
...
void ThreadControllerImpl::SetDefaultTaskRunner(
    scoped_refptr<SingleThreadTaskRunner> task_runner) {
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetDefaultTaskRunner");
  if (!message_loop_){
  TRACE_EVENT0("dorsal", "ThreadControllerImpl::SetDefaultTaskRunner_END");
    return;}
