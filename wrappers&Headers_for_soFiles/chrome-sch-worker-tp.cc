...
void trpoint_jsontolttng(int thread_id, char* phase, char* category_group_enabled, char* name, char* scope, unsigned long long id, unsigned long long bind_id, int num_args, char* arg_types, const unsigned long long arg_values) {
	...
	else if (strcmp(name, "SequenceManagerImpl::TakeTask_END") == 0 )
		tracepoint(chrome, SequenceManagerImpl_TakeTask_END, thread_id, phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_types, arg_values);
	...
	else if (strcmp(name, "ContentFaviconDriver::DidUpdateFaviconURL") == 0 )
		tracepoint(chrome, ContentFaviconDriver_DidUpdateFaviconURL, thread_id, phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_types, arg_values);
	...
	else if (strcmp(name, "TaskTracker::RunTask") == 0 )
		tracepoint(chrome, TaskTracker_RunTask, thread_id, phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_types, arg_values);

	else if (strcmp(name, "TaskTracker::RunTask_END") == 0 )
		tracepoint(chrome, TaskTracker_RunTask_END, thread_id, phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_types, arg_values);
