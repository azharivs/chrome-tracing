...
static void * tppHandle4 = NULL;//sva
static int soLoaded4 = 0;//sva
static void * tppHandle5 = NULL;//sva
static int soLoaded5 = 0;//sva
static void * tppHandle6 = NULL;//sva
static int soLoaded6 = 0;//sva
static void * tppHandle7 = NULL;//sva
static int soLoaded7 = 0;//sva
static void * tppHandle8 = NULL;//sva
static int soLoaded8 = 0;//sva
static void * tppHandle9 = NULL;//sva
static int soLoaded9 = 0;//sva
static void * tppHandle10 = NULL;//sva
static int soLoaded10 = 0;//sva
static void * tppHandle11 = NULL;//sva
static int soLoaded11 = 0;//sva
...
void (*tp_ptr)(const char*);
        ...
        if (!soLoaded){
            tppHandle = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle) {
        ...
        *(void **) (&tp_ptr) = dlsym(tppHandle, "_Z35GpuChannelHost_Send_StartWrapUniquePc");
        ...
        (*tp_ptr)((const char*) "GpuChannelHost::Send&StartWrapUnique");
...
void (*tp_ptr3)(const char*);
        char *error3;

        if (!soLoaded3){
            tppHandle3 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle3) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr3) = dlsym(tppHandle3, "_Z22BelongsToCurrentThreadPc");
        ...
        (*tp_ptr3)((const char*) "base::WrapUnique_END&StartDCHECK(!io_thread_->BelongsToCurrentThread)");
...
void (*tp_ptr4)(const char*);
        char *error4;

        if (!soLoaded4){
            tppHandle4 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle4) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr4) = dlsym(tppHandle4, "_Z19message_set_unblockPc");

        if ((error4 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error4);
        }

       
        (*tp_ptr4)((const char*) "DCHECK(!io_thread_->BelongsToCurrentThread)_END&StartMessage->set_unblock");
...
void (*tp_ptr5)(const char*);
        char *error5;

        if (!soLoaded5){
            tppHandle5 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle5) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr5) = dlsym(tppHandle5, "_Z15message_is_syncPc");

        if ((error5 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error5);
        }

       
        (*tp_ptr5)((const char*) "Message->set_unblock_END&Start!message->is_sync");
...
void (*tp_ptr6)(const char*);
        char *error6;

        if (!soLoaded6){
            tppHandle6 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle6) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr6) = dlsym(tppHandle6, "_Z18io_thread_PostTaskPc");

        if ((error6 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error6);
        }

       
        (*tp_ptr6)((const char*) "!message->is_sync_END&Startio_thread_->PostTask");
...
void (*tp_ptr7)(const char*);
        char *error7;

        if (!soLoaded7){
            tppHandle7 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle7) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr7) = dlsym(tppHandle7, "_Z22io_thread_PostTask_ENDPc");

        if ((error7 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error7);
        }

       
        (*tp_ptr7)((const char*) "io_thread_->PostTask_END");
...
void (*tp_ptr8)(const char*);
        char *error8;

        if (!soLoaded8){
            tppHandle8 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle8) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr8) = dlsym(tppHandle8, "_Z34Startbase_WaitableEvent_done_eventPc");

        if ((error8 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error8);
        }

       
        (*tp_ptr8)((const char*) "Startbase::WaitableEvent done_event");
...
void (*tp_ptr9)(const char*);
        char *error9;

        if (!soLoaded9){
            tppHandle9 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle9) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr9) = dlsym(tppHandle9, "_Z31IPC_PendingSyncMsg_pending_syncPc");

        if ((error9 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error9);
        }

       
        (*tp_ptr9)((const char*) "Startbase::WaitableEvent done_event_END&StartIPC::PendingSyncMsg pending_sync");
...
void (*tp_ptr10)(const char*);
        char *error10;

        if (!soLoaded10){
            tppHandle10 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle10) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr10) = dlsym(tppHandle10, "_Z19io_thread_PostTask2Pc");

        if ((error10 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error10);
        }

       
        (*tp_ptr10)((const char*) "IPC::PendingSyncMsg pending_sync_END&Startio_thread_->PostTask");
...
void (*tp_ptr11)(const char*);
        char *error11;

        if (!soLoaded11){
            tppHandle11 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle11) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr11) = dlsym(tppHandle11, "_Z28pending_sync_done_event_WaitPc");

        if ((error11 = dlerror()) != NULL)  {
             fprintf(stderr, "00: Upon dlsym: %s\n", error11);
        }

       
        (*tp_ptr11)((const char*) "io_thread_->PostTask_END&Startpending_sync.done_event->Wait");
...
void (*tp_ptr2)(const char*);
        char *error2;

        if (!soLoaded2){
            tppHandle2 = dlopen("/home/majid/Documents/chromium/src/lib6.so", RTLD_NOW);// | RTLD_GLOBAL | RTLD_NODELETE);
            if (!tppHandle2) {
                 fprintf(stderr, "00: Upon dlopen: %s\n", dlerror());
            }
            else {
        ...
        *(void **) (&tp_ptr2) = dlsym(tppHandle2, "_Z56GpuChannelHost_Send_END_pending_sync_done_event_Wait_ENDPc");
        ...
        (*tp_ptr2)((const char*) "GpuChannelHost::Send_END&pending_sync.done_event->Wait_END");
